...
- [Java 内存区域详解](#java-内存区域详解)
    ...
    - [二 运行时数据区域](#二-运行时数据区域)
        ...
        - [2.5 方法区](#25-方法区)
            ...
            - [2.5.2 为什么要将永久代 (PermGen) 替换为元空间 (MetaSpace) 呢?](#252-为什么要将永久代-permgen-替换为元空间-metaspace-呢)
...
![JAVA8内存结构](./images/jvm_structure.jpg)
...
**Java 内存可以粗糙的区分为堆内存（Heap）和栈内存 (Stack),其中栈就是现在说的虚拟机栈，或者说是虚拟机栈中局部变量表部分。** （实际上，Java 虚拟机栈是由一个个`栈帧`组成，而每个栈帧中都拥有：局部变量表、操作数栈、动态链接、方法出口信息。）
...
![String-Pool-Java](./images/String_Pool.png)
