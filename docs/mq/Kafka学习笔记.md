...
golang 版本实现
1.  go.mod版本 github.com/Shopify/sarama v1.26.4
...
func main() {
	...
	producer, err := sarama.NewSyncProducer([]string{"10.12.179.175:9092"}, config)
	if err != nil {
		panic(err)
	}
	...
	msg := &sarama.ProducerMessage {
		...
		Value: sarama.StringEncoder("this is a good test,hello chaoge!!"),
	}
	//发送消息
...
func main() {
	consumer, err := sarama.NewConsumer([]string{"10.12.179.175:9092"}, nil)
	if err != nil {
		panic(err)
	}
	//Partitions(topic):该方法返回了该topic的所有分区id
	partitionList, err := consumer.Partitions("sun")
	if err != nil {
		panic(err)
	}
	...
	for partition := range partitionList {
		...
		pc, err := consumer.ConsumePartition("sun", int32(partition), sarama.OffsetNewest)
		if err != nil {
			panic(err)
		}
		defer pc.AsyncClose()
