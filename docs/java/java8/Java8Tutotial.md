...
Java 8使我们能够通过使用default关键字向接口添加非抽象的方法实现。此功能也称为[虚拟扩展方法](http://stackoverflow.com/a/24102730)。
...
Java 语言设计者投入了大量的精力来思考如何实现现有的函数友好的支持Lambda 。最终采用的方法是：增加函数式接口的概念。 **“函数式接口”是指仅仅只包含一个抽象方法，但是可以有多个非抽象方法（默认方法）**像这样的接口，可以被隐式的转换为lambda表达式。`java.lang.Runnable` 与 `java.util.concurrent.Callable` 是函数式接口最典型的两个例子。java 8 新增了一种特殊的注解，`@FunctionalInterface`  但是这个注解通常情况下不是必须的，只要接口中只包含一个抽象方法，虚拟机会自动判断该接口为函数式接口。一般建议在接口上使用 `@FunctionalInterface` 注解进行声明，这样，编译器如果发现你标注了这个注解的接口有多于一个的抽象方法，就会报错。
...
{
	public static void main(String[] args) {
		test("abc",(s)->s.length() == 3);
	}
	public static void test(String var, Predicate<String> predicate)
	{
		boolean flag = predicate.test(var);
...
{
	public static void main(String[] args) {
		int a = test("123",Integer::parseInt);
		System.out.println(a); //int 123
	}
	public static int test(String str, Function<String,Integer> function)
	{
...
{
	public static void main(String[] args) {
		test(2,System.out::print);
	}
	public static void test(int num,Consumer<Integer> consumer)
	{
		consumer.accept(num);
	}
}
...
{
	public static void main(String[] args) {
		int re = test("ac","abc",(a,b)->a.length()-b.length());
		System.out.println(re); // -1
	}
	public static int test(String s1, String s2, Comparator<String> comparator)
	{
...
![来源公众号](https://user-gold-cdn.xitu.io/2018/11/28/167598cd2e17b8ec?w=258&h=258&f=jpeg&s=27334)
